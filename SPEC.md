...
Saat start, bot membandingkan jam lokal dengan jam server exchange memakai titik tengah round trip. Panggilan pertama ke host memuat resolusi DNS dan jabat tangan TLS, jadi satu panggilan pemanasan dibuang dulu, lewat klien dan sesi HTTP yang sama dengan yang mengukur. Lalu diambil minimal exchange.time_sync_samples sampel; sampel dengan rtt di atas exchange.time_sync_max_rtt_ms (300 ms) DIBUANG, bukan dipakai, karena ketidakpastian estimasi titik tengah sekitar rtt/2 dan data pemilik menunjukkan pada rtt besar estimasi itu melacak setengah rtt, bukan jam (rtt 1990 ms memberi selisih -1018 ms, rtt 74 ms memberi -60 ms, jam sebenarnya -25 sampai -33 ms). Kalau belum ada sampel yang cukup cepat, pengambilan diteruskan sampai exchange.time_sync_max_attempts. Yang dipakai adalah sampel cepat dengan rtt terkecil. Dua keadaan dipisahkan tegas. Jam melenceng: ada sampel cepat (pengukuran valid, rtt di bawah batas rtt) dan selisihnya melebihi exchange.max_time_drift_ms, fatal dengan pesan yang menyebut selisih, rtt terbaik, dan jumlah sampel terpakai dari jumlah percobaan. Pengukuran tidak konklusif: tidak ada satu pun sampel yang cukup cepat; ini jaringan yang lambat, bukan jam yang salah, tidak pernah disebut "pengukuran valid", dan tidak pernah dilaporkan sebagai jam yang salah. Untuk kasus kedua yang dilihat adalah batas yang benar-benar dipakai exchange, recv_window_ms: kalau batas atas selisih (|titik tengah| + rtt/2 dari sampel tercepat) masih di bawahnya, jam tidak mungkin membuat exchange menolak request dan bot lanjut dengan peringatan; kalau sudah menyentuhnya, bot berhenti dengan pesan bahwa yang gagal adalah pengukurannya. Batas max_time_drift_ms sendiri tidak dinaikkan. Log selalu menyebut tiga estimator pada sampel terbaik: awal kirim (t0 dikurangi stempel server), titik tengah, dan akhir terima (t1 dikurangi stempel server); pada hari pengukuran lambat itu estimator akhir terima stabil di -21 sampai -23 ms untuk semua sampel, yang berarti jedanya terjadi sebelum request sampai ke server (antrean atau pembukaan koneksi), bukan sesudahnya. Keputusan tetap memakai titik tengah pada sampel cepat, yang kesalahannya paling besar setengah batas rtt.
...
Uji pipa sekali jalan (`tradebot pipe-test`, pipe-test.command) membuktikan jalur order end to end dengan uang asli berukuran minimum tanpa menunggu sinyal, ditunggui operator. Urutan dikunci tanpa loop: preflight lengkap (satu gagal berarti tidak ada yang dikirim); tampilkan ukuran, perkiraan rupiah, perkiraan biaya bolak-balik, minta ketikan UJI PIPA persis; beli market ukuran minimum dengan niat ke jurnal (fsync) sebelum kirim; pasang STOP_LOSS_LIMIT lapis 2 dan verifikasi lewat fetch_open_orders, bukan respons create (gagal berarti langsung jual kembali; posisi tidak pernah ditinggalkan tanpa stop); tunggu 60 detik; batalkan stop dulu, verifikasi hilang, baru jual market; verifikasi flat tanpa order terbuka; rekonsiliasi ledger lewat fetch_my_trades sampai fee terisi. Kalau gagal di tengah, akun dirapikan lebih dulu (order terbuka dibatalkan, sisa dijual; kalau jual gagal, stop pengaman dipasang lagi) dan laporan menyebut keadaan akhir serta tindakan manual. Protokol niat-kirim-hasil disalin dari runner, bukan diimpor, supaya runner, RiskManager, dan strategi tidak berubah; kunci runner yang sama dipakai supaya tidak berjalan bersamaan dengan bot. Ukuran tidak bisa dinaikkan lewat flag, withdrawal tidak pernah disentuh, live.enabled tidak diperlukan karena ketikan UJI PIPA adalah persetujuan untuk satu putaran itu.
...
│   │   └── compare.py       # fill paper/live vs backtest per trade, uji bias satu arah
│   ├── live/
...
│   │   └── pipe_test.py     # uji pipa sekali jalan: beli minimum, stop lapis 2, batal, jual
│   └── cli.py
└── tests/
```
