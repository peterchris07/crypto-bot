...
Kalau jam mesin melenceng lebih dari batas di config, perintah gagal dengan pesan yang menyebut selisihnya, rtt terbaik, dan jumlah sampel; sinkronkan jam sistem lalu ulangi. Kalau yang muncul adalah "pengukuran jam tidak konklusif", jam Anda tidak salah: jaringan terlalu lambat untuk mengukurnya (rtt terbaik terlalu besar), coba lagi saat koneksi lebih stabil. Pengukuran memakai satu panggilan pemanasan yang dibuang lalu beberapa sampel; sampel dengan rtt di atas exchange.time_sync_max_rtt_ms (300 ms) dibuang, pengambilan diteruskan sampai exchange.time_sync_max_attempts kalau belum ada yang cepat, dan yang dipakai sampel cepat dengan rtt terkecil. Jadi koneksi dingin atau jaringan yang tersendat tidak pernah dinilai sebagai jam yang buruk. Log menyebut tiga estimator (awal kirim, titik tengah, akhir terima) pada sampel terbaik supaya terlihat mana yang stabil di jaringan Anda. Kalau pair di config tidak ada lagi di venue, perintah gagal dengan pesan yang menyebut pair dan jumlah pasar per quote yang tersedia.
...
Di antara live-setup dan live-start ada `pipe-test.command` (`tradebot pipe-test`): uji pipa sekali jalan dengan uang asli berukuran minimum, tanpa menunggu sinyal strategi, ditunggui dari layar. Urutannya dikunci: preflight lengkap (satu gagal berarti tidak ada yang dikirim), tampilkan ukuran order, perkiraan rupiah, dan perkiraan biaya bolak-balik lalu minta ketik UJI PIPA persis, beli market ukuran minimum (niat ke jurnal dan fsync sebelum kirim), pasang stop lapis 2 dan verifikasi lewat fetch_open_orders (kalau tidak terpasang, langsung jual kembali), tunggu 60 detik sambil menampilkan posisi, harga, dan stop, batalkan stop dulu dan verifikasi hilang, baru jual market, verifikasi flat tanpa order terbuka, rekonsiliasi ledger sampai fee terisi. Laporan akhir satu layar: harga beli dan jual, selisih, fee per baris ledger dan perkiraan per komponen, untung atau rugi bersih dalam USDT dan rupiah, bukti stop lapis 2 (id dan hasil fetch), baris jurnal dan ledger terkait, lalu vonis JALUR ORDER TERBUKTI atau GAGAL di langkah berapa. Kalau gagal di tengah, laporan menyebut apakah Anda masih memegang BTC, order terbuka yang tertinggal, dan apa yang harus dilakukan manual di aplikasi; akun flat dan bersih diutamakan di atas menyelesaikan ujinya. Ukuran tidak bisa dinaikkan lewat flag, satu putaran tanpa loop, tidak menyentuh RiskManager, runner, atau strategi, dan memakai kunci runner yang sama supaya tidak jalan bersamaan dengan bot. Tidak butuh live.enabled: ketikan UJI PIPA adalah persetujuan untuk satu putaran itu saja.

Yang tidak pernah terjadi di jalur ini: kunci muncul di layar, log, argumen proses, atau file selain .env; order dikirim sebelum preflight lulus; live aktif tanpa ketikan SAYA SIAP (atau UJI PIPA untuk satu putaran uji pipa).
